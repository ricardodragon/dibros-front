                ...
                <tbody>
                    {props.variations.map((variacao, index) => <>
                        <tr key={(index+1)*100}>
                            ...
                            <td><input id={index+"qtd_disponivel_varicao"} type="number" onChange={event=>props.onChange(props.variations.map((v,i)=>i===index?{...v, available_quantity:event.target.value}:v))} disabled={props.disabled} className={"form-control form-control-sm"} value={variacao.available_quantity}/></td>
