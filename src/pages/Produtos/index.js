        ...
        <div>
            <form className="mt-4" onSubmit={submit}>                
                <fieldset id="usuario" className="p-2" style={{overflow:"hidden", borderRadius:"0.9em"}}>                    
                    ...
                    {values.produto.imagemPath&&<img alt="" style={{width:"8em", height:"8em"}} src={process.env.REACT_APP_MELI_DOMAIN+values.produto.imagemPath}/>}
                    {values.produto.imagem&&<img alt="" style={{width:"8em", height:"8em"}} src={URL.createObjectURL(values.produto.imagem)}/>}            
                    ...
                    <tbody> 
                        ...
                        {values.produtos.map(p=>
                            <tr key={p.id} style={{cursor:"pointer", whiteSpace: "nowrap"}} onClick={event=>{event.preventDefault();window.scrollTo(0, 0);setValues({...values, produto:{...p, loja:p.lojaDTO}});document.getElementsByClassName("conteudo")[0].scrollTo(0, 0)}}>
                                <td>{p.id}</td>
                                <td><img alt="" style={{width:"2em", height:"2em"}} src={process.env.REACT_APP_MELI_DOMAIN+p.imagemPath}/></td>                            
