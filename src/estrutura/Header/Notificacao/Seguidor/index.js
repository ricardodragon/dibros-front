...
function Seguidor(props){
    ...
    return values.usuario?
        <div style={{padding:'1%', textAlign:'left'}}>
            <Link style={{display:'inline', verticalAlign:'top', width:'50%'}} to={"/perfil/"+values.usuario.id}><img alt="Imagem perfil user" src={`${host}/auth/usuarios/imagem/${values.usuario.id}/${values.usuario.imagem}?Authorization=${localStorage.getItem("token")}`} style={{borderRadius: "50%", width:"2.7em", height:"2.7em"}}/></Link>                                      
            ...
            <button style={{display:'inline', backgroundColor:'#858080', color:'white', borderRadius:'7%', cursor:'pointer'}} onClick={event=>{event.preventDefault();axios.put("/loja/seguidores/"+values.usuario.id).then(props.setNotificacao(props.index, 'SEGUIDO_ACEITO'))}}>aceitar</button>
