...
function Colaborador(props){
    ...
    return values.loja?
        <div style={{padding:'1%', textAlign:'left'}} className='notificacao-check'>
            <Link style={{display:'inline', verticalAlign:'top', width:'50%'}} to={"/perfil/"+values.loja.id}><img alt="Imagem perfil user" src={`${host}/loja/lojas/imagem/${values.loja.id}/${values.loja.imagem}?Authorization=${localStorage.getItem("token")}`} style={{borderRadius: "50%", width:"2.7em", height:"2.7em"}}/></Link>                                      
